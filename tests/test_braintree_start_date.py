...
class BraintreeStartDateTest(BraintreeBaseTest):

    start_date_1 = "2022-06-19T00:00:00Z"
    ...
    def test_run(self):
        ...
        self.assertGreater(
            sum(record_count_by_stream_1.values()),
            sum(record_count_by_stream_2.values()),
        )
        ...
        for stream in expected_streams:
            ...
            with self.subTest(stream=stream):
                ...
                if self.is_incremental(stream):
                    ...
                    bookmark_keys_list_1 = [
                        message.get("data").get(next(iter(expected_bookmark_keys)))
                        for message in synced_records_1.get(stream).get("messages")
                        if message.get("action") == "upsert"
                    ]
                    bookmark_keys_list_2 = [
                        message.get("data").get(next(iter(expected_bookmark_keys)))
                        for message in synced_records_2.get(stream).get("messages")
                        if message.get("action") == "upsert"
                    ]
