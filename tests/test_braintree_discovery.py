...
class DiscoveryTest(BraintreeBaseTest):
    """Test whether tap discovery mode and metadata conforms to standards or not."""
    ...
    def test_run(self):
        ...
        • Verify stream names follow naming convention streams should only have lowercase alphas
            and underscores
        • verify there is only 1 top level breadcrumb
        • verify replication key(s)
        • verify primary key(s)
        • verify that if there is a replication key we are doing INCREMENTAL replication otherwise FULL TABLE replication
        • verify the actual replication matches our expected replication method
        • verify that primary, replication and replication keys are given the inclusion of automatic.
        ...
        for stream in streams_to_test:
            with self.subTest(stream=stream):
                ...
                # Ensure that the catalog is found for a given stream
                catalog = next(
                    iter(
                        [
                            catalog
                ...
                # verify that if there is a replication key we are doing INCREMENTAL replication otherwise FULL TABLE replication
                if expected_replication_keys is not None:
                    self.assertEqual(expected_replication_method, "INCREMENTAL")
                else:
                    self.assertEqual(expected_replication_method, "FULL_TABLE")
